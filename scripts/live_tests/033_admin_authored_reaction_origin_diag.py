#!/usr/bin/env python3
"""Live test for accepted admin-authored reaction provenance in diagnostics."""
...
def _submit_unique_admin_authored_lighting(
    client: HAFlowClient,
    flow_id: str,
    *,
    room_id: str,
    ...
    weekday_candidates = [str(i) for i in range(7)]
    for weekday in weekday_candidates:
        for scheduled_time in _candidate_times():
    ...
    raise AssertionError("unable to find a free lighting slot for admin-authored proposal")


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Heima admin-authored reaction provenance diagnostics live test"
    )
    ...
    try:
        ...
        room_id = room_ids[0]
        light_entity = "light.test_heima_living_main"

        step = _submit_unique_admin_authored_lighting(
            client,
            flow_id,
            room_id=room_id,
        ...
        _assert(
            "Template: lighting.scene_schedule.basic" in details,
            "proposal details do not expose template id before accept",
        )
        ...
        print("PASS: admin-authored accepted reaction is visible in reactions diagnostics")
        return 0
    finally:
        time.sleep(0.2)
        try:
