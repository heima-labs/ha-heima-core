#!/usr/bin/env python3
"""Live test for admin-authored room darkness lighting assist flow."""
...
def _submit_unique_admin_authored_room_darkness_lighting(
    client: HAFlowClient,
    flow_id: str,
    *,
    room_id: str,
) -> dict[str, Any]:
    primary_name_candidates = ["room_lux", "lux", "darkness"]
    for primary_signal_name in primary_name_candidates:
        step = client.options_flow_configure(
            flow_id,
            {
                ...
                "light_entities": ["light.test_heima_studio_main", "light.test_heima_studio_spot"],
                "action": "on",
                "brightness": 190,
    ...
    raise AssertionError("unable to find a free darkness lighting slot for admin-authored proposal")


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Heima admin-authored room darkness lighting flow live test"
    )
    ...
    try:
        ...
        _assert(
            ...
            f"room.darkness_lighting_assist template not exposed: {template_ids}",
        )

        step = client.options_flow_configure(
            flow_id, {"template_id": "room.darkness_lighting_assist.basic"}
        )
        ...
        step = _submit_unique_admin_authored_room_darkness_lighting(
            client,
            flow_id,
            room_id=room_id,
        )
        ...
        _assert(
            "Template: room.darkness_lighting_assist.basic" in details,
            "proposal details do not expose room.darkness_lighting_assist template id",
        )
        _assert(
            ("Soglia buio:" in details) or ("Darkness threshold:" in details),
            "proposal details do not expose darkness threshold",
        )
        _assert(
            ("Luci configurate:" in details) or ("Configured lights:" in details),
            "proposal details do not expose configured lights",
        )
        ...
        print("PASS: admin-authored room darkness lighting flow created an accepted reaction")
        return 0
    finally:
        time.sleep(0.2)
        try:
