#!/usr/bin/env python3
"""True live E2E test for cross-domain room signal assist learning.

The Docker lab fixture provides 4 historical bathroom humidity/ventilation
episodes. This script performs one real bathroom occupancy + humidity burst +
temperature rise + ventilation activation sequence through Home Assistant
entities so CrossDomainPatternAnalyzer can emit a pending `room_signal_assist`
proposal without using seeded runtime commands.
"""
...
def _find_room_signal_assist(diag: dict[str, Any]) -> tuple[str, dict[str, Any]] | None:
    ...
    for proposal in proposals:
        if not isinstance(proposal, dict):
            continue
        if proposal.get("type") != "room_signal_assist":
            continue
        if proposal.get("status") != "pending":
            continue
        description = str(proposal.get("description") or "")
        if not description.startswith("bathroom:"):
            continue
...
def _wait_for_room_signal_proposal(
    client: HAClient, entry_id: str, previous: int, timeout_s: int, poll_s: float
    ...
    while time.time() < deadline:
        ...
        found = _find_room_signal_assist(diag)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Heima true live room-signal-assist test")
    ...
    required = [
        ...
        "binary_sensor.heima_occupancy_bagno",
    ...
    if existing is not None:
        ...
        print(f"PASS: room signal assist proposal already pending [preexisting] id={proposal_id}")
        return 0
    ...
    print("Marking bathroom occupied and recomputing snapshot context...")
    client.call_service(
        "input_boolean",
        "turn_on",
    ...
    _recompute(client)
    client.wait_state("binary_sensor.heima_occupancy_bagno", "on", args.timeout_s, args.poll_s)
    ...
    after = _wait_for_state_change_growth(client, entry_id, baseline, args.timeout_s, args.poll_s)
    print(f"state_change events after live bathroom sequence: {after}")

    print("Reloading Heima config entry to trigger proposal run...")
    client.call_service("homeassistant", "reload_config_entry", {"entry_id": entry_id})

    proposal_id, _proposal, mode = _wait_for_room_signal_proposal(
        client, entry_id, proposals_before, args.timeout_s, args.poll_s
    )
    print(f"PASS: live room signal assist proposal ready [{mode}] id={proposal_id}")
    return 0
