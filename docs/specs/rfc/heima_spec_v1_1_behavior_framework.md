...
**Status:** Implemented/Partial — behavior base implemented; reactive path evolved in dedicated spec
**Last Verified Against Code:** 2026-03-11

> **Implementation status (2026-03-09)**:
>
> - `HeimaBehavior` infrastructure (base class, `on_snapshot` dispatch, exception isolation,
>   `on_options_reloaded`, `diagnostics`) is **implemented** as of v0.3.0.
> - `apply_filter` on `HeimaBehavior` is available as an extension point but **no concrete
>   built-in behaviors use it**. Reason: without a `tags` field on `ApplyStep`, it cannot safely
>   distinguish auto-generated from user-intent steps. The Constraints Layer handles blocking
