...
async def test_event_recorder_presence_arrive_and_depart():
    hass = _FakeHass()
    store = _FakeStore()
    behavior = EventRecorderBehavior(hass, store)  # type: ignore[arg-type]
    ...
    transitions = [e.transition for e in store.events if e.event_type == "presence"]
...
async def test_event_recorder_house_state_transition():
    ...
    assert changes[0].from_state == "home"
    assert changes[0].to_state == "working"
