...
@pytest.mark.asyncio
async def test_security_mismatch_generic_only_emits_generic_event(monkeypatch):
    monkeypatch.setattr("custom_components.heima.runtime.engine.time.monotonic", lambda: 100.0)
    engine = _engine(
        _base_options(
            {
                "enabled_event_categories": ["security", "people"],
                "security_mismatch_policy": "strict",
                "security_mismatch_event_mode": "generic_only",
                "security_mismatch_persist_s": 999,
            }
        ),
        {
    ...
    assert "security.armed_away_but_home" not in _event_types(engine)
    assert "security.mismatch" in _event_types(engine)


@pytest.mark.asyncio
async def test_security_mismatch_dual_emit_emits_specific_and_generic(monkeypatch):
    monkeypatch.setattr("custom_components.heima.runtime.engine.time.monotonic", lambda: 100.0)
    engine = _engine(
        _base_options(
            {
                "enabled_event_categories": ["security", "people"],
                "security_mismatch_policy": "strict",
                "security_mismatch_event_mode": "dual_emit",
                "security_mismatch_persist_s": 999,
            }
        ),
        {
    ...
    assert "security.armed_away_but_home" in event_types
    assert "security.mismatch" in event_types
