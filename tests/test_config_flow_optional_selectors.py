...
def test_notifications_schema_excludes_legacy_routes_field():
    flow = _flow()
    ...
    assert "routes" not in schema.schema
...
def test_notifications_payload_defaults_event_categories_when_missing():
    ...
    assert "routes" not in normalized
    ...
    assert normalized["security_mismatch_event_mode"] == "explicit_only"
...
def test_notifications_payload_normalizes_security_mismatch_event_mode():
    flow = _flow()
    normalized = flow._normalize_notifications_payload(
        {
            "routes": [],
            ...
            "security_mismatch_event_mode": "invalid",
            "security_mismatch_persist_s": 42,
            "dedup_window_s": 60,
            "rate_limit_per_key_s": 300,
        }
...
def test_notifications_payload_strips_routes_field():
    flow = _flow()
    normalized = flow._normalize_notifications_payload(
        {
            "routes": ["mobile_app_legacy"],
    ...
    assert "routes" not in normalized
    assert normalized["recipients"] == {"stefano": ["mobile_app_phone_stefano"]}
