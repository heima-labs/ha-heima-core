...
def build_room_inventory_summary(
    ...
    for room in rooms:
        item = _build_room_item(
            room, entity_registry=entity_registry, device_registry=device_registry
        )
    ...
    return {
        ...
        "rooms_with_inventory": sum(
            1 for item in room_items if int(item.get("inventory_entity_total") or 0) > 0
        ),
        "rooms": room_items,
    }
...
def _build_room_item(
    ...
    *,
    ...
    by_domain = Counter(
        entity_id.split(".", 1)[0] for entity_id in inventory_entities if "." in entity_id
    )
...
def _is_suggested_occupancy_entity(entity_id: str) -> bool:
    ...
    if domain == "binary_sensor":
        return any(
            token in name for token in ("motion", "presence", "occupancy", "mmwave", "radar")
        )
    if domain == "sensor":
