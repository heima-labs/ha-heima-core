...
async def async_register_services(hass: HomeAssistant) -> None:
    async def _handle_command(call: ServiceCall) -> None:
        ...
        if command == "mute_reaction":
            reaction_id = _require_target_value(params or target, "reaction_id")
            matched = False
            for coordinator in coordinators:
                if coordinator.engine.mute_reaction(reaction_id):
                    matched = True
            if not matched:
                raise ServiceValidationError(f"Reaction '{reaction_id}' not found")
            return

        if command == "unmute_reaction":
            reaction_id = _require_target_value(params or target, "reaction_id")
            matched = False
            for coordinator in coordinators:
                if coordinator.engine.unmute_reaction(reaction_id):
                    matched = True
            if not matched:
                raise ServiceValidationError(f"Reaction '{reaction_id}' not found")
            return
