"""RoomContextModule — approved learned house-state signals from room device context."""

from __future__ import annotations

from collections import defaultdict
from dataclasses import dataclass
from typing import Any

from ....const import HOUSE_STATES_LEARNED_CONTEXT_ELIGIBLE
from ...room_context import RoomDeviceContext, deserialize_room_device_context
from ..approval_store import (
    HOUSE_STATE_PROPOSAL_TYPE,
    house_state_context_key,
    house_state_context_snapshot,
)
from ..base import HeimaLearningModule, InferenceContext, SnapshotHistoryStore
from ..signals import HouseStateSignal, Importance

_MIN_SUPPORT = 20
_CONFIDENCE_THRESHOLD = 0.65
_PROPOSABLE_HOUSE_STATES = frozenset(HOUSE_STATES_LEARNED_CONTEXT_ELIGIBLE)
_MODULE_CONTEXT_TAG = "room_context"

RoomContextPattern = frozenset[tuple[str, bool, bool]]


@dataclass(frozen=True)
class _ModelEntry:
    weekday: int
    hour_bucket: int
    rooms: tuple[str, ...]
    pattern: RoomContextPattern
    predicted_state: str
    total: int
    count: int
    confidence: float
    context_key: str
    context_snapshot: dict[str, Any]


@dataclass(frozen=True)
class LearnedRoomContextCandidate:
    """Proposal-first learned room-context house-state candidate."""

    proposal_type: str
    context_key: str
    context_snapshot: dict[str, Any]
    predicted_state: str
    support: int
    total: int
    confidence: float


class RoomContextModule(HeimaLearningModule):
    """Learns P(house_state | room context pattern, weekday, hour bucket)."""

    module_id = "room_context"

    def __init__(
        self,
        *,
        min_support: int = _MIN_SUPPORT,
        confidence_threshold: float = _CONFIDENCE_THRESHOLD,
    ) -> None:
        self._min_support = max(1, int(min_support))
        self._confidence_threshold = max(0.0, min(float(confidence_threshold), 1.0))
        self._model: dict[tuple[int, int, RoomContextPattern], _ModelEntry] = {}
        self._approved_context_keys: set[str] = set()
        self._rejected_context_keys: set[str] = set()
        self._ready = False
        self._analyzed_snapshots = 0

    def sync_approval_state(self, approved: set[str], rejected: set[str]) -> None:
        """Replace the in-memory approval snapshot used by sync inference."""
        self._approved_context_keys = _normalized_keys(approved)
        self._rejected_context_keys = _normalized_keys(rejected)

    async def analyze(self, store: SnapshotHistoryStore) -> None:
        """Compute room-context probabilities from post-Phase-X snapshots only."""
        counts: dict[tuple[int, int, RoomContextPattern], dict[str, int]] = defaultdict(
            lambda: defaultdict(int)
        )
        rooms_by_key: dict[tuple[int, int, RoomContextPattern], tuple[str, ...]] = {}
        analyzed = 0

        for snapshot in store.snapshots():
            if not snapshot.house_state:
                continue
            context = deserialize_room_device_context(
                getattr(snapshot, "room_device_context", {}) or {}
            )
            if not context:
                continue
            pattern = _pattern_from_context(
                room_device_context=context,
                room_occupancy=snapshot.room_occupancy,
            )
            if not pattern:
                continue
            key = (int(snapshot.weekday), int(snapshot.minute_of_day) // 60, pattern)
            counts[key][str(snapshot.house_state)] += 1
            rooms_by_key[key] = _rooms_from_pattern(pattern)
            analyzed += 1

        self._model = {}
        for key, state_counts in counts.items():
            total = sum(state_counts.values())
            predicted_state = max(state_counts, key=lambda state: state_counts[state])
            count = state_counts[predicted_state]
            confidence = count / total if total > 0 else 0.0
            weekday, hour_bucket, pattern = key
            rooms = rooms_by_key.get(key, ())
            learning_context = _learning_context(pattern)
            context_key = house_state_context_key(
                weekday=weekday,
                hour_bucket=hour_bucket,
                rooms=rooms,
                anyone_home=True,
                predicted_state=predicted_state,
                learning_context=learning_context,
            )
            self._model[key] = _ModelEntry(
                weekday=weekday,
                hour_bucket=hour_bucket,
                rooms=rooms,
                pattern=pattern,
                predicted_state=predicted_state,
                total=total,
                count=count,
                confidence=confidence,
                context_key=context_key,
                context_snapshot=house_state_context_snapshot(
                    weekday=weekday,
                    hour_bucket=hour_bucket,
                    rooms=rooms,
                    anyone_home=True,
                    predicted_state=predicted_state,
                    learning_context=learning_context,
                ),
            )

        self._analyzed_snapshots = analyzed
        self._ready = True

    def infer(self, context: InferenceContext) -> list[HouseStateSignal]:
        if not self._ready:
            return []
        pattern = _pattern_from_context(
            room_device_context=context.room_device_context,
            room_occupancy=context.room_occupancy,
        )
        if not pattern:
            return []
        entry = self._model.get((context.weekday, context.minute_of_day // 60, pattern))
        if entry is None:
            return []
        if entry.total < self._min_support:
            return []
        if entry.confidence < self._confidence_threshold:
            return []
        if entry.context_key not in self._approved_context_keys:
            return []

        return [
            HouseStateSignal(
                source_id=self.module_id,
                confidence=entry.confidence,
                importance=Importance.SUGGEST,
                ttl_s=600,
                label=f"{entry.predicted_state} room_context h={context.minute_of_day // 60}",
                predicted_state=entry.predicted_state,
            )
        ]

    def generate_candidates(self) -> list[LearnedRoomContextCandidate]:
        if not self._ready:
            return []

        candidates: list[LearnedRoomContextCandidate] = []
        for entry in self._model.values():
            if entry.predicted_state not in _PROPOSABLE_HOUSE_STATES:
                continue
            if entry.total < self._min_support:
                continue
            if entry.confidence < self._confidence_threshold:
                continue
            if entry.context_key in self._approved_context_keys:
                continue
            if entry.context_key in self._rejected_context_keys:
                continue
            candidates.append(
                LearnedRoomContextCandidate(
                    proposal_type=HOUSE_STATE_PROPOSAL_TYPE,
                    context_key=entry.context_key,
                    context_snapshot=dict(entry.context_snapshot),
                    predicted_state=entry.predicted_state,
                    support=entry.count,
                    total=entry.total,
                    confidence=entry.confidence,
                )
            )
        return sorted(candidates, key=lambda candidate: candidate.context_key)

    def diagnostics(self) -> dict[str, Any]:
        return {
            "module_id": self.module_id,
            "ready": self._ready,
            "slot_count": len(self._model),
            "approved_context_keys": len(self._approved_context_keys),
            "rejected_context_keys": len(self._rejected_context_keys),
            "analyzed_snapshots": self._analyzed_snapshots,
            "min_support": self._min_support,
            "confidence_threshold": self._confidence_threshold,
        }


def _pattern_from_context(
    *,
    room_device_context: dict[str, RoomDeviceContext],
    room_occupancy: dict[str, bool],
) -> RoomContextPattern:
    items: list[tuple[str, bool, bool]] = []
    for room_id, occupied in sorted(room_occupancy.items()):
        if not occupied:
            continue
        ctx = room_device_context.get(str(room_id))
        if ctx is None:
            continue
        items.append((str(room_id), bool(ctx.media_on), bool(ctx.work_activity)))
    return frozenset(items)


def _rooms_from_pattern(pattern: RoomContextPattern) -> tuple[str, ...]:
    return tuple(sorted(room_id for room_id, _media_on, _work_activity in pattern))


def _learning_context(pattern: RoomContextPattern) -> dict[str, Any]:
    return {
        "module": _MODULE_CONTEXT_TAG,
        "room_context_pattern": [
            {
                "room_id": room_id,
                "media_on": media_on,
                "work_activity": work_activity,
            }
            for room_id, media_on, work_activity in sorted(pattern)
        ],
    }


def _normalized_keys(keys: set[str]) -> set[str]:
    return {str(key).strip() for key in keys if str(key).strip()}
