...
class _ReactionFormHelpersMixin:
    ...
    def _contextual_lighting_preset_options() -> dict[str, str]:
        return {
            ...
            for preset_id, payload in _ReactionFormHelpersMixin._contextual_lighting_preset_catalog().items()
        }

    @staticmethod
    def _contextual_lighting_preset_label(preset: str) -> str:
        catalog = _ReactionFormHelpersMixin._contextual_lighting_preset_catalog()
    ...
    def _contextual_lighting_policy_json(*, preset: str, light_entities: list[str]) -> str:
        contract = _ReactionFormHelpersMixin._contextual_lighting_contract_from_preset(
            preset=preset,
            light_entities=light_entities,
        )
    ...
    def _contextual_lighting_preset_from_cfg(cfg: dict[str, Any]) -> str:
        light_entities = _ReactionFormHelpersMixin._contextual_lighting_light_entities_from_cfg(cfg)
        current = {
            "profiles": dict(cfg.get("profiles") or {}),
            "rules": list(cfg.get("rules") or []),
            "default_profile": str(cfg.get("default_profile") or "").strip(),
        ...
        for preset in _ReactionFormHelpersMixin._contextual_lighting_preset_options():
            if (
                _ReactionFormHelpersMixin._contextual_lighting_contract_from_preset(
                    preset=preset,
                    light_entities=light_entities,
                )
                == current
